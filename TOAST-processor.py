...
def int2col(arr, dict):
    return [dict[v] for v in arr]
def col2int(arr, dict):
    # if not isinstance(arr, list):
    #     arr = [arr]
    return [dict.index(v) for v in arr]  # Only for lists/arrays, not for scalars
def scoreChunk(chunk, orig):
    score = np.sum(np.array(orig) == np.array(chunk))
    return score  # /sum(x!=0 for x in chunk) # 0 is 'na'

colDict = ['na', 'blu', 'wht', 'red', 'gry', 'grn', 'blk', 'yel', 'stp']
...
for ts in sorted(results):  # Dangerous: results dict must be sorted by key (i.e. timestamp)
    ...
    for chunk, group in resChunks:
        # print(chunk, group)
        pat = col2int(group['color'], colDict)
        N = len(pat)
